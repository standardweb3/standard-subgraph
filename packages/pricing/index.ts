import {
  ...
  USDC_WETH_PAIR_FIRST_LIQUIDITY_BLOCK,
  BIG_INT_ZERO,
  SUSHI_USDC_PAIR_ADDRESS,
  SUSHI_USDC_PAIR_FIRST_LIQUIDITY_BLOCK,
} from 'const'
...
export function getUSDRate(token: Address, block: ethereum.Block): BigDecimal {
  ...
  if (token != USDC_ADDRESS) {
    if (block.number.lt(USDC_WETH_PAIR_FIRST_LIQUIDITY_BLOCK)) return BIG_DECIMAL_ZERO
    ...
    const reserve0 = reserves.value0.toBigDecimal().div(BIG_DECIMAL_1E6)

    const reserve1 = reserves.value1.toBigDecimal().div(BIG_DECIMAL_1E18)

    const ethPriceUSD = reserve0.div(reserve1)
...
export function getSushiPrice(block: ethereum.Block): BigDecimal {
  if (
    SUSHI_WETH_PAIR_FIRST_LIQUIDITY_BLOCK.equals(BIG_INT_ZERO) ||
    block.number.lt(SUSHI_WETH_PAIR_FIRST_LIQUIDITY_BLOCK)
  ) {
    // If before sushi-eth pair creation and liquidity added, return zero
    return BIG_DECIMAL_ZERO
  } else if (
    SUSHI_USDC_PAIR_FIRST_LIQUIDITY_BLOCK.equals(BIG_INT_ZERO) ||
    block.number.lt(SUSHI_USDC_PAIR_FIRST_LIQUIDITY_BLOCK)
  ) {
    // Else if before sushi-usdt pair creation (get price from eth sushi-eth pair above)
