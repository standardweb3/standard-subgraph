...
export function updateVaultManagerRunningStat(block:ethereum.Block): VaultManagerRunningStat {
    ...
    if (stats === null) {
        ...
        stats.vaultManager = manager.id

        stats.ammReserveCollateralUSD = BIG_DECIMAL_ZERO
        stats.historicCollateralizedUSD = BIG_DECIMAL_ZERO
    ...
    stats.historicBorrowedUSD = manager.historicBorrowed.times(stats.stablecoinPrice)
...
export function updateVaultRunningStat(gIndex: BigInt, block: ethereum.Block): VaultRunningStat {
    ...
    stats.currentCollateralizedUSD = vault.currrentCollateralized.times(stats.collateralPrice)
...
export function updateCollateralVaultRunningStat(collateral: Address, block: ethereum.Block): CollateralVaultRunningStat {
    ...
    stats.historicCollateralizedUSD = vault.historicCollateralized.times(stats.collateralPrice)

    stats.stablecoinPrice = getAssetPrice(MTR_ADDRESS)
    stats.currentBorrowedUSD = vault.currentBorrowed.times(stats.stablecoinPrice)
    stats.historicBorrowedUSD = vault.historicBorrowed.times(stats.stablecoinPrice)
